          ...
          {activeCitation?.source ? (
            ...
            <BrainConstellation
              ...
              onSourceSelect={(selectedSource) => {
                const source = typeof selectedSource === "number"
                  ? latestSources?.[selectedSource - 1]
                  : selectedSource;
                if (!source) return;
                const citationIndex = latestSources.findIndex((item) => String(item?.id) === String(source?.id));
                updateSession({
                  activeCitation: {
                    citationNumber: citationIndex >= 0 ? citationIndex + 1 : null,
                    source,
